...
paramsTxt =['nu', 'T']
upper_bound = [5000, 5000/(2.0*1032)]
lower_bound = [0.5, 10/(2.0*1032)]
...
opt_prob.addVar('nu','c',lower=lower_bound[0],upper=upper_bound[0],value=p1[0])
opt_prob.addVar('T','c',lower=lower_bound[1],upper=upper_bound[1],value=p1[1])
opt_prob.addObj('f')
...
if myrank == 0:
    ...
    print 'Nref:',Nref
    scaledParams = [Nref*popt[0],2*Nref*popt[1],2*Nref]
...
if myrank == 0:      
    ...
    print ""
    print "NeA\ttG\tNe0"
    print "%s\t%s\t%s" %(Nref, scaledParams[1], scaledParams[0])
